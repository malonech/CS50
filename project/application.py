...
@login_required
def index():
    recentruns = db.execute("SELECT Distance, Duration, Pace, Calories, Timestamp FROM rundata WHERE id IS (?) ORDER BY datetime(Timestamp) DESC LIMIT 10", session["user_id"])
...
def entry():
    if request.method == "GET":
        ...
        distance = round(float(request.form.get("distKm")),2)
        durationmin = int(request.form.get("minutes"))
        durationsec = int(request.form.get("seconds"))
        now = strftime('%Y-%m-%d %H:%M:%S')

        markers = request.form.get("marker")

        #for point in markerhistory:
        #    print(f"{point}")

        decimalduration = round(durationmin + durationsec / 60,2)
        pace = round(decimalduration / distance,2)
...
def history():
    if request.method == "GET":
        runs = db.execute("SELECT Distance, Duration, Pace, Calories, Timestamp FROM rundata WHERE (?) = id ORDER BY datetime(Timestamp) DESC", session["user_id"])
        bestpace = db.execute("SELECT Distance, Timestamp, Pace FROM rundata WHERE Pace = (SELECT MIN(Pace) FROM rundata WHERE id IS (?))", session["user_id"])
        longestrun = db.execute("SELECT Distance, Pace, Timestamp FROM rundata WHERE Distance = (SELECT MAX(Distance) FROM rundata WHERE (?) = id)", session["user_id"])
        return render_template("history.html", runs = runs, bestpace = bestpace, longestrun=longestrun)
    else:
        filter_parameter = str(request.form.get("parameter"))
        upperbound = float(request.form.get("upperbound"))
        lowerbound = float(request.form.get("lowerbound"))

        bestpace = db.execute("SELECT Distance, Timestamp, Pace FROM rundata WHERE Pace = (SELECT MIN(Pace) FROM rundata WHERE id IS (?))", session["user_id"])
        longestrun = db.execute("SELECT Distance, Pace, Timestamp FROM rundata WHERE Distance = (SELECT MAX(Distance) FROM rundata WHERE (?) = id)", session["user_id"])

        if filter_parameter == 'Distance':
            runs = db.execute("SELECT Distance, Duration, Pace, Calories, Timestamp FROM rundata WHERE (?) = id AND (Distance BETWEEN (?) AND (?)) ORDER BY datetime(Timestamp) DESC"
                                , session["user_id"], lowerbound, upperbound)
        elif filter_parameter == 'Duration':
            runs = db.execute("SELECT Distance, Duration, Pace, Calories, Timestamp FROM rundata WHERE (?) = id AND (Duration BETWEEN (?) AND (?)) ORDER BY datetime(Timestamp) DESC"
                                , session["user_id"], lowerbound, upperbound)
        else:
            runs = db.execute("SELECT Distance, Duration, Pace, Calories, Timestamp FROM rundata WHERE (?) = id AND (Pace BETWEEN (?) AND (?)) ORDER BY datetime(Timestamp) DESC"
                                , session["user_id"], lowerbound, upperbound)
